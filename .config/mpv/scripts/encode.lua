...
function ffmpeg_progress(args, length)
    print(shell_escape(args))
    local res = io.popen(shell_escape(args))
    while true do
        ...
        if line:find("^out_time_us") ~= nil then
            currtime = tonumber(line:sub(13, -1))
            mp.osd_message(string.format("encoding progress: %.2f%%", currtime / 10000 / length))
        end
    end
    local status
    status, _, _ = res:close()
    print(status)
    if status then
...
function start_encoding(from, to, settings)
    local args = {
        ...
        "-loglevel", "panic", "-nostats", "-hide_banner", "-progress", "-"
    }
