...
if (nix profile list | lines | any {|e| $e =~ $'($env.HOME)/.config/nix'}) {
} else {
    nix profile install $'path:($env.HOME)/.config/nix'
}

source ~/.config/nushell/omp.nu
use ~/.config/nushell/git-completions.nu *
use ~/.config/nushell/cargo-completions.nu *
use ~/.config/nushell/nix-completions.nu *
use ~/.config/nushell/git.nu *

def u [] {
    sudo eix-sync
    sudo emerge -auUDNv @world
}
